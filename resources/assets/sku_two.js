(function () {
    ...
    SKU.prototype.init = function () {
        ...
        // 选择sku的类型（单规格/多规格）
        _this.warp.find('.sku_two_attr_select .btn').click(function () {
            let _dom = $(this);
            if (!_dom.hasClass('btn-success')) {
                ...
                if (_dom.hasClass('Js_single_btn')) {
                    // 点击了单规格
                    // 隐藏多规格编辑DOM
                    _this.warp.find('.sku_two_attr_key_val,.sku_two_edit_warp').hide();
                } else if (_dom.hasClass('Js_many_btn')) {
                    // 点击了多规格
                    // 显示多规格编辑DOM
                    _this.warp.find('.sku_two_attr_key_val,.sku_two_edit_warp').show();
                }
            }
            _this.processSku()
        });
        ...
        // 绑定属性值添加事件
        _this.warp.find('.sku_two_attr_key_val').on('click', '.Js_add_attr_val', function () {
        ...
        // 绑定属性值移除事件
        _this.warp.find('.sku_two_attr_key_val').on('click', '.Js_remove_attr_val', function () {
        ...
        _this.warp.find('.Js_add_attr_name').click(function () {
            let html = '<tr>' +
                    ...
                    '</td>' +
                    '</tr>';
            _this.warp.find('.sku_two_attr_key_val tbody').append(html)
        });

        // 绑定移除属性名事件
        _this.warp.find('.sku_two_attr_key_val').on('click', '.Js_remove_attr_name', function () {
            console.log('移除属性名');
        ...
        // 绑定input变化事件
        _this.warp.find('.sku_two_attr_key_val tbody').on('change', 'input', _this.getSkuAttr.bind(_this));
        _this.warp.find('.sku_two_edit_warp tbody').on('keyup', 'input', _this.processSku.bind(_this));
        // 绑定select变化事件
        _this.warp.find('.sku_two_edit_warp tbody').on('change', 'select', _this.processSku.bind(_this));

        // 统一库存
        _this.warp.find('.sku_two_edit_warp thead').on('keyup', 'input.Js_stock', function () {
            _this.commonStock = $(this).val();
            _this.warp.find('.sku_two_edit_warp tbody td[data-field="stock"] input').val(_this.commonStock);
            _this.processSku()
        });

        // 统一价格
        _this.warp.find('.sku_two_edit_warp thead').on('keyup', 'input.Js_price', function () {
            _this.commonPrice = $(this).val();
            _this.warp.find('.sku_two_edit_warp tbody td[data-field="price"] input').val(_this.commonPrice);
            _this.processSku()
        });

        // 统一定金
        _this.warp.find('.sku_two_edit_warp thead').on('keyup', 'input.Js_deposit', function () {
            _this.commonGoodsDeposit = $(this).val();
            _this.warp.find('.sku_two_edit_warp tbody td[data-field="goods_deposit"] input').val(_this.commonGoodsDeposit);
            _this.processSku()
        });

        // SKU图片上传
        _this.warp.find('.sku_two_edit_warp tbody').on('click', '.Js_sku_upload', function () {
            _this.upload($(this))
        });

        // 清空SKU图片
        _this.warp.find('.sku_two_edit_warp tbody').on('click', '.Js_sku_del_pic', function () {
            let td = $(this).parent();
            td.find('input').val('');
        ...
        if (old_val) {
            ...
            if (old_val.type === 'many') {
                // 多规格
                _this.warp.find('.sku_two_attr_select .Js_many_btn').trigger('click');

                // 处理规格名
                let attr_names = old_val.attrs;
                let tbody = _this.warp.find('.sku_two_attr_key_val tbody');
    ...
    SKU.prototype.getSkuAttr = function () {
        let attr = {}; // 所有属性
        let _this = this;
        let trs = _this.warp.find('.sku_two_attr_key_val tbody tr');
        trs.each(function () {
            let tr = $(this);
    ...
    SKU.prototype.SKUForm = function (default_sku, type = 0) {
        ...
        if (attr_names.length === 0) {
            _this.warp.find('.sku_two_edit_warp tbody').html(' ');
            _this.warp.find('.sku_two_edit_warp thead').html(' ');
        } else {
            // 渲染表头
            let thead_html = '<tr>';
            ...
            if (default_sku) {
                // 填充数据
                default_sku.forEach(function (item_sku, index) {
                    let tr = _this.warp.find('.sku_two_edit_warp tbody tr').eq(index);
    ...
    SKU.prototype.processSku = function () {
        let _this = this;
        let sku_json = {};
        sku_json.type = _this.warp.find('.sku_two_attr_select .btn.btn-success').attr('data-type');
        ...
        _this.warp.find('.Js_two_sku_input').val(JSON.stringify(sku_json));
    };
